...
#zhujie
Sx1 = np.array([[0, 1], [1, 0]], dtype=complex)  # single-site S^x
Sy1 = np.array([[0, -1j], [1j, 0]], dtype=complex)  # single-site S^y

H1 = np.array([[0, 0], [0, 0]], dtype=complex)  # single-site portion of H is zero
#zhujie
def H2(Sx1, Sy1, Sx2, Sy2,g):  # two-site part of H
    """Given the operators S^x and S^y on two sites in different Hilbert spaces
...
def enlarge_block(block,g):
    ...
    enlarged_operator_dict = {
        "H": kron(o["H"], identity(model_d)) + kron(identity(mblock), H1) + H2(o["conn_Sx"], o["conn_Sy"], Sx1, Sy1,g),
...
def single_dmrg_step(sys, env, m,g):
    ...
    #zhujie return eigenvalues for entropy calculation
    truncated_evals = np.array([x[0] for x in possible_eigenstates[:my_m]])
    ...
    return newblock, energy, truncated_evals#zhujie
...
def infinite_system_algorithm(L, m,g):
    ...
    while 2 * block.length < L:
        ...
        #zhujie
        block, energy, eival = single_dmrg_step(block, block, m=m, g=g)
...
def finite_system_algorithm(L, m_warmup, m_sweep_list,g):
    ...
    while 2 * block.length < L:
        ...
        #zhujie
        block, energy,eival  = single_dmrg_step(block, block, m=m_warmup,g=g)
        #print("E/L =", energy / (block.length * 2))
        block_disk["l", block.length] = block
        block_disk["r", block.length] = block
    ...
    for m in m_sweep_list:
        ...
        # Perform a single DMRG step.
            ...
            sys_block, energy, eival = single_dmrg_step(sys_block, env_block, m=m,g=g)
            #print("E/L =", energy / L)
            # Save the block from this step to disk.
            block_disk[sys_label, sys_block.length] = sys_block
            # zhujie calculate entanglement entropy and record
            S_L = -np.sum(eival[eival>0] * np.log(eival[eival>0]))
            SL_dict[sys_block.length] = S_L  # system length as key 

            # Check whether we just completed a full sweep.
            if sys_label == "l" and 2 * sys_block.length == L:
                break  # escape from the "while True" loop
    #zhujie
    return energy,SL_dict

if __name__ == "__main__":
    np.set_printoptions(precision=10, suppress=True, threshold=10000, linewidth=300)

    #infinite_system_algorithm(L=100, m=20)
    energy, SL_dict=finite_system_algorithm(L=20, m_warmup=10, m_sweep_list=[10, 20, 30, 40, 40],g=1)
    print(energy)
    print(SL_dict)
